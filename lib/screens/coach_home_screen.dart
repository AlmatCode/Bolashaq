...
class _CoachHomeScreenState extends State<CoachHomeScreen>
  ...
  // Данные тренера
  String? _coachId;
  List<CoachSection> _coachSections = [];

  // Состояние приложения
  Map<String, List<SectionApplication>> _sectionApplications = {};
  Map<String, bool> _sectionLoading = {};
  Map<String, String> _sectionFilters = {};
  Map<String, List<SectionMember>> _sectionMembers = {};
  Map<String, Map<String, AttendanceRecord>> _sectionAttendance = {};
    ...
    _realtimeChannel = _supabase
      ...
      callback: (payload) {
        if (mounted) {
          ...
          if (sectionId != null && _sectionAttendance.containsKey(sectionId)) {
            _loadSectionAttendance(sectionId, _sectionMembers[sectionId] ?? []);
          }
        }
      },
    )
  ...
  Future<void> _refreshCurrentData() async {
    if (_coachSections.isNotEmpty && _tabController.index < _coachSections.length) {
      final sectionId = _coachSections[_tabController.index].id;
      await _loadSectionApplications(sectionId);
      await _loadSectionMembers(sectionId);
    }
  }
  ...
  Future<void> _loadCoachData() async {
    try {
      ...
      final sectionsResponse = await _supabase
          ...
          .select('''
            id, title, type, category, coach_name, 
            schedule, location, capacity, current_members,
            is_active, description, coach_id
          ''')
          .eq('type', 'sport')
          .or('coach_id.eq.${user.id},coach_name.ilike.%${user.email}%')
          .eq('is_active', true)
          .order('title');

      if (sectionsResponse != null && sectionsResponse.isNotEmpty) {
        final List<CoachSection> sections = sectionsResponse
            .map((section) =>
            CoachSection.fromJson(Map<String, dynamic>.from(section)))
            .toList();

        // Инициализируем состояние для каждой секции
        for (var section in sections) {
          _sectionFilters[section.id] = 'pending';
          _sectionLoading[section.id] = false;
        }

        setState(() {
          _coachSections = sections;
        });

        _tabController = TabController(
          length: _coachSections.length,
          vsync: this,
        );

        // Загружаем данные для первой вкладки
        if (_coachSections.isNotEmpty) {
          await _loadSectionApplications(_coachSections.first.id);
          await _loadSectionMembers(_coachSections.first.id);
        }
      }
    } catch (error) {
      debugPrint('Error loading coach data: $error');
      _showSnackBar('Ошибка загрузки данных', isError: true);
    }
  }

  Future<void> _loadSectionApplications(String sectionId) async {
    try {
      setState(() => _sectionLoading[sectionId] = true);
      ...
      if (applicationsResponse != null) {
        final List<SectionApplication> applications = [];
        int total = 0, pending = 0, approved = 0, rejected = 0;

        for (var app in applicationsResponse) {
          try {
            final application =
            SectionApplication.fromJson(Map<String, dynamic>.from(app));
        ...
        setState(() {
          _sectionApplications[sectionId] = applications;
      ...
      setState(() => _sectionLoading[sectionId] = false);
      _showSnackBar('Ошибка загрузки заявок', isError: true);
    }
  }

  Future<void> _loadSectionMembers(String sectionId) async {
    try {
      ...
      if (membersResponse != null) {
        final List<SectionMember> members = [];
        for (var member in membersResponse) {
          members.add(SectionMember.fromJson(Map<String, dynamic>.from(member)));
        }

        setState(() {
          _sectionMembers[sectionId] = members;
        });

        // Загружаем посещаемость для этих участников
        await _loadSectionAttendance(sectionId, members);
      }
    } catch (error) {
      debugPrint('Error loading section members: $error');
    }
  }

  Future<void> _loadSectionAttendance(
      String sectionId, List<SectionMember> members) async {
    try {
      ...
      if (attendanceResponse != null) {
        for (var record in attendanceResponse) {
          final attendance = AttendanceRecord.fromJson(
      ...
      for (var member in members) {
        if (!attendanceMap.containsKey(member.userId)) {
          attendanceMap[member.userId] = AttendanceRecord(
            id: '',
            sectionId: sectionId,
      ...
      setState(() {
        _sectionAttendance[sectionId] = attendanceMap;
      });
    } catch (error) {
  ...
  Future<void> _updateApplicationStatus(
    ...
    try {
      await _supabase.from('section_applications').update({
        ...
        'reviewed_by': _coachId,
      }).eq('id', applicationId);

      if (newStatus == 'approved') {
        await _increaseSectionMembers(sectionId);
        await _addStudentToSection(sectionId, applicantId);
      }
  ...
  Future<void> _updateAttendance(
      String sectionId, String userId, AttendanceStatus status) async {
    try {
      final formattedDate = DateFormat('yyyy-MM-dd').format(_selectedDate);
      final existingRecord = _sectionAttendance[sectionId]?[userId];

      if (existingRecord != null && existingRecord.id.isNotEmpty) {
        // Обновляем существующую запись
        await _supabase.from('attendance_records').update({
          'status': status.name,
          'recorded_by': _coachId,
        ...
        if (response != null && response.isNotEmpty) {
          ...
          setState(() {
            _sectionAttendance[sectionId]?[userId] = AttendanceRecord(
              id: newId,
              sectionId: sectionId,
              userId: userId,
              date: _selectedDate,
      ...
      setState(() {
        final record = _sectionAttendance[sectionId]?[userId];
        if (record != null) {
          _sectionAttendance[sectionId]?[userId] = AttendanceRecord(
            id: record.id.isNotEmpty ? record.id : '',
  ...
  Future<void> _saveAllAttendance(String sectionId) async {
    try {
      final attendanceRecords = _sectionAttendance[sectionId] ?? {};
  ...
  Future<void> _generateAttendanceReport(String sectionId) async {
    try {
      final section = _coachSections.firstWhere((s) => s.id == sectionId);
      final members = _sectionMembers[sectionId] ?? [];
      final attendance = _sectionAttendance[sectionId] ?? {};
...
''';
  ...
  Future<void> _increaseSectionMembers(String sectionId) async {
    try {
      ...
      final currentMembers = _toInt(sectionResponse['current_members']);
      final capacity = _toInt(sectionResponse['capacity']);

      if (capacity > 0 && currentMembers >= capacity) {
        throw Exception('Секция заполнена');
      }
      ...
      debugPrint('Error increasing section members: $error');
      rethrow;
    }
  }

  Future<void> _addStudentToSection(String sectionId, String studentId) async {
    try {
      ...
      debugPrint('Error adding student to section: $error');
    }
  }
  ...
  Future<void> _showDatePicker() async {
    ...
    if (picked != null && picked != _selectedDate) {
      ...
      // Обновляем посещаемость для всех секций
      for (var section in _coachSections) {
        final members = _sectionMembers[section.id];
        if (members != null) {
          await _loadSectionAttendance(section.id, members);
        }
      }
    }
  }
  ...
  Future<void> _showApplicationDetails(SectionApplication application) async {
    await showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildDetailRow('Секция:', application.sectionTitle),
              _buildDetailRow(
                  'Тренер:',
                  application.sectionType == 'sport'
                      ? _getCoachName(application.sectionTitle)
                      : ''),
  ...
  Future<void> _showRejectDialog(SectionApplication application) async {
  ...
  Future<void> _updateApplicationStatusWithReason(
    ...
    try {
      ...
      final currentSectionId = _coachSections[_tabController.index].id;
      await _loadSectionApplications(currentSectionId);
  ...
  String _getCoachName(String sectionTitle) {
    final section = _coachSections.firstWhere(
  ...
  Widget _buildAttendanceTab(String sectionId) {
    final members = _sectionMembers[sectionId] ?? [];
    final attendance = _sectionAttendance[sectionId] ?? {};
    final section = _coachSections.firstWhere(
          (s) => s.id == sectionId,
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () => _saveAllAttendance(sectionId),
              ...
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () => _generateAttendanceReport(sectionId),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final record = attendance[member.userId];
              return _buildAttendanceCard(member, record, sectionId);
            },
          ),
        ),
      ],
  ...
  Widget _buildAttendanceCard(
      SectionMember member, AttendanceRecord? record, String sectionId) {
    final status = record?.status ?? AttendanceStatus.absent;
  ...
  Widget _buildFilterChips(
      String currentFilter, Function(String) onFilterChanged) {
  ...
  Widget _buildApplicationCard(SectionApplication application) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 6),
      child: InkWell(
  ...
  Widget _buildApplicationsTab(String sectionId) {
    final applications = _sectionApplications[sectionId] ?? [];
    final currentFilter = _sectionFilters[sectionId] ?? 'pending';
    ...
    // Фильтруем заявки
    List<SectionApplication> filteredApplications = applications;
    ...
    return RefreshIndicator(
      onRefresh: () => _loadSectionApplications(sectionId),
      child: ListView.builder(
        padding: const EdgeInsets.only(bottom: 20),
            ...
            child: Column(
              children: [
                TabBar(
                  ...
                  isScrollable: true,
                  onTap: (index) async {
                    final sectionId = _coachSections[index].id;
                    if (_sectionApplications[sectionId] == null) {
                      await _loadSectionApplications(sectionId);
                    }
                    if (_sectionMembers[sectionId] == null) {
                      await _loadSectionMembers(sectionId);
                    }
                  },
                  tabs: _coachSections
        ...
        body: Column(
          children: [
            ...
            Expanded(
              child: TabBarView(
                controller: _tabController,
                children: _coachSections.map((section) {
                  return TabBarView(
                    children: [
                      // Вкладка заявок
                      Column(
                        children: [
                          ...
                          _buildFilterChips(
                            _sectionFilters[section.id] ?? 'pending',
                                (filter) => setState(() {
                              _sectionFilters[section.id] = filter;
                            }),
                          ),
                          Expanded(
                            child: _buildApplicationsTab(section.id),
                          ),
                        ],
                      ),
                      // Вкладка посещаемости
                      _buildAttendanceTab(section.id),
                    ],
                  );
                }).toList(),
              ),
...
class CoachSection {
  ...
  factory CoachSection.fromJson(Map<String, dynamic> json) {
    return CoachSection(
      id: json['id']?.toString() ?? '',
...
class SectionApplication {
  ...
  factory SectionApplication.fromJson(Map<String, dynamic> json) {
    final profiles = json['profiles'] is Map
        ? Map<String, dynamic>.from(json['profiles'])
        : <String, dynamic>{};
